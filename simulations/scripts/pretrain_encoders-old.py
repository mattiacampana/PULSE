...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--input-channels", type=int, help="Legacy NPZ override; inferred for HDF5 datasets.")
...
def _load(args: argparse.Namespace) -> tuple[Dataset, dict[str, Any], list[str], np.ndarray | None]:
    ...
    dataset = ModelReadyDataset(data_path, branches)
...
def main() -> None:
    ...
    encoder = build_encoder(str(encoder_name), **_encoder_kwargs(str(encoder_name), branches, dataset, embedding_dim, args.input_channels)).to(device)
    ...
    print(f"  data: {args.data} | samples: {len(dataset):,} ({len(train_indices):,} train, {len(eval_indices):,} eval)")
    print(f"  branches: {', '.join(branches)} | classes: {num_classes} | embedding: {embedding_dim}")
    ...
    metadata = {
        "format_version": 3, "encoder_name": str(encoder_name), "embedding_dim": embedding_dim,
