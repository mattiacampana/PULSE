...
| CIFAR-10 | RGB image `[3, 32, 32]` | 10 simulated nodes | tiny 2-D CNN |
| Fashion-MNIST | grayscale image `[1, 28, 28]` | 10 simulated nodes | tiny 2-D CNN |
...
### Controlled image-benchmark mode

CIFAR-10 and Fashion-MNIST have no real user identity. Their adapters therefore
use a sample-level protocol designed for pretraining followed by opportunistic
simulation. For every class, a class-balanced subset of the official training
split is reserved for backbone pretraining and a second, disjoint subset for
pretraining validation. All remaining official training examples are assigned
to simulated nodes through a class-wise Dirichlet partition. The official test
split is independently partitioned across the same nodes and is never used for
training. Thus, an image cannot occur in both pretraining and simulation.

The default `dirichlet_alpha: 0.1` creates strong label skew. Minimum train and
test counts prevent empty nodes, and the global `seed` makes every partition
reproducible.
...
For image benchmarks, pixels are scaled to `[0, 1]`; no statistics from
simulation examples are used to normalize pretraining examples, or vice versa.
...
Install the optional torchvision dependency once:

```bash
python -m pip install -e '.[vision]'
```

Both adapters accept these YAML fields:
...
The same fields apply to `fashion_mnist`. They deliberately replace the
user-count split fields, which apply only to datasets with real subjects.
