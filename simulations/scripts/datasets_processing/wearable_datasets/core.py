...
def write_dataset(samples: list[Sample], output_dir: Path, descriptor: dict[str, object], split: SplitConfig) -> dict[str, object]:
    ...
    manifest = pd.DataFrame({
        ...
        "split": [membership.get(item.subject_id, "unused") for item in samples],
    ...
    _write_json(output_dir / "splits.json", {"seed": split.seed, "selection_unit": "subject", "split_mode": "subject" if split.enabled else "full", **groups})
