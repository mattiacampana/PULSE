...
def main() -> None:
    ...
    print(
        "[prepare] preprocessing_version=%s | normalization=%s"
        % (_PREPROCESSING_VERSION, _NORMALIZATION)
    )
...
def _prepare_uci_har(raw_dir: Path, output_path: Path, stats_dir: Path, force_download: bool, force_preprocess: bool) -> None:
    if output_path.exists() and not force_preprocess:
        ...
        if "normalization" in archive.files and str(archive["normalization"]) == _NORMALIZATION:
            ...
            return
        raise ValueError(
            f"{output_path} was generated with an older preprocessing scheme; "
            "rerun with --force-preprocess."
        )
    ...
    np.savez_compressed(
        ...
        class_names=np.asarray(_uci_class_names()),
        normalization=np.asarray(_NORMALIZATION),
        metadata=np.asarray(_metadata_json("uci_har")),
    )
...
def _prepare_hhar(raw_dir: Path, output_path: Path, stats_dir: Path, args: argparse.Namespace, force_download: bool, force_preprocess: bool) -> None:
    ...
    np.savez_compressed(
        ...
        normalization=np.asarray(_NORMALIZATION),
        metadata=np.asarray(_metadata_json("hhar_by_user")),
    )
...
def _verify_saved_archive(output_path: Path) -> None:
    """Fail early if a save did not contain the required preprocessing fields."""
    with np.load(output_path, allow_pickle=False) as archive:
        required = {"x", "y", "class_names", "normalization", "metadata"}
        missing = required.difference(archive.files)
        if missing:
            raise RuntimeError("Saved archive is incomplete (%s): missing %s" % (output_path, sorted(missing)))
        print("[prepare] verified %s | keys=%s | normalization=%s" % (
            output_path, list(archive.files), str(archive["normalization"])
        ))


def _ensure_dataset(destination: Path, url: str, expected_directory: Optional[str], force_download: bool) -> Path:
...
def _find_root(destination: Path, expected_directory: Optional[str]) -> Path:
...
def _normalize_windows(x: np.ndarray) -> np.ndarray:
    ...
    A global UCI-to-HHAR normalization is not suitable here: the two sources
    are recorded by different phones and have incompatible raw scales.  This
    operation is applied identically to each window, using no labels and no
    statistics shared across users or datasets.
    """
...
def _window_hhar_records(frame: pd.DataFrame, window_size: int, stride: int, max_gap_ms: Optional[float]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def _archive_split_counts(archive: np.lib.npyio.NpzFile) -> Optional[Tuple[int, int]]:
...
def _write_uci_statistics(
    ...
    split_counts: Optional[Tuple[int, int]],
...
def _class_rows(dataset: str, labels: np.ndarray, class_names: List[str], split: Optional[np.ndarray]) -> List[Dict[str, object]]:
    rows = []
